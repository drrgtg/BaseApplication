...
var path = require('path'),
    ...
    srcDir = 'BaseApplication_demo/dirName',
    dstDir = path.join(BUILT_PRODUCTS_DIR, FULL_PRODUCT_NAME),
    dstWwwDir = path.join(dstDir, 'BaseApplication_demo/dirName');
...
var cmd;
if(!!COPY_HIDDEN) {
	  cmd = 'rsync -Lra "' + srcDir + '" "' + dstDir + '"';
    code = shell.exec(cmd).code;
} else {
	  cmd = 'rsync -Lra --exclude="- .*" "' + srcDir + '" "' + dstDir + '"';
    code = shell.exec(cmd).code;
}
...
if(code !== 0) {
    console.error('Error occured on copying template. Code: ' + code);
    process.exit(3);
}
